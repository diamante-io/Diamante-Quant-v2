...
// Logger interface is defined in consensus_time.go
...
func NewOptimizer(c types.Consensus, logger Logger) *Optimizer {
	return &Optimizer{
		...
		lastBlockTime: common.ConsensusNow(),
		ema:           0,
		logger:        logger,
	}
}
...
func (o *Optimizer) logPerformanceMetrics(
	...
	var gossipDelay string = "nil"
	var votingThreshold string = "nil"
	var dposSetSize string = "nil"
	var pohTickDelay string = "nil"

	if o.consensus != nil {
		// Retrieve lachesis and check its underlying value.
		lachesis := o.consensus.GetLachesis()
		if lachesis != nil && !reflect.ValueOf(lachesis).IsNil() {
			gossipDelay = fmt.Sprintf("%v", lachesis.GetGossipDelay())
			votingThreshold = fmt.Sprintf("%.2f", lachesis.GetVotingThreshold())
		}

		// Retrieve DPoS and check.
		dpos := o.consensus.GetDPoS()
		if dpos != nil && !reflect.ValueOf(dpos).IsNil() {
			dposSetSize = fmt.Sprintf("%d", dpos.GetSetSize())
		}

		// Retrieve PoH and check.
		poh := o.consensus.GetPoH()
		if poh != nil && !reflect.ValueOf(poh).IsNil() {
			pohTickDelay = fmt.Sprintf("%v", poh.GetTickDelay())
		}
	}
...
func (o *Optimizer) GetOptimizationStats() *dtypes.OptimizationStats {
	o.mu.RLock()
	defer o.mu.RUnlock()

	metrics := o.collectPerformanceMetrics()
	stats := &dtypes.OptimizationStats{
		PredictedLoad:    o.PredictLoad(),
	...
	if len(o.samples) > 0 {
		stats.CurrentLoad = o.samples[len(o.samples)-1]
	} else {
		stats.CurrentLoad = 0.5
	}
	...
	if o.consensus != nil {
		...
		poh := o.consensus.GetPoH()
		if lachesis != nil {
			stats.LachesisGossipDelay = lachesis.GetGossipDelay()
			stats.LachesisVotingThreshold = lachesis.GetVotingThreshold()
		}
		if dpos != nil {
			stats.DPoSSetSize = dpos.GetSetSize()
			stats.DPoSEpochDuration = time.Duration(dpos.GetEpochDuration())
		}
		if poh != nil {
			stats.PoHTickDelay = poh.GetTickDelay()
		}
	}
	return stats
}
...
func (o *Optimizer) ResetOptimization() {
	...
	o.lastBlockTime = common.ConsensusNow()
	o.ema = 0
