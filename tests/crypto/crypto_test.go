...
func TestKyberKeyGen(t *testing.T) {
	...
	scheme := crypto.KyberSchemeFromLevel(crypto.KyberLevel1024)
	if scheme == nil {
		logger.Error("Failed to get Kyber scheme")
		t.Fatalf("KyberSchemeFromLevel(%d) returned nil", crypto.KyberLevel1024)
	}
	logger.WithField("level", crypto.KyberLevel1024).Debug("Kyber scheme initialized")

	kyber := crypto.NewKyberCrypto(scheme, logger)
...
func TestKyberEncapsDecaps(t *testing.T) {
	...
	if scheme == nil {
		logger.Error("Failed to get Kyber scheme")
		t.Fatalf("Failed to get kyber scheme for %d", crypto.KyberLevel1024)
	}
...
func TestKyberAESGCM(t *testing.T) {
	...
	encStart := time.Now()
	encData, err := crypto.EncryptWithShared(plain, ss)
	...
	// Decrypt
	decStart := time.Now()
	recovered, err := crypto.DecryptWithShared(encData, ss2)
...
func TestCryptoManagerBasic(t *testing.T) {
	...
	start := time.Now()
	cm, err := crypto.NewCryptoManager(crypto.KyberLevel1024, crypto.DilithiumLevel3, logger)
...
func BenchmarkCryptoOperations(b *testing.B) {
	logger := getTestLogger()
	cm, _ := crypto.NewCryptoManager(crypto.KyberLevel1024, crypto.DilithiumLevel3, logger)
...
func TestSecurityLevelIntegration(t *testing.T) {
	...
	levels := []struct {
		...
		{crypto.KyberLevel512, crypto.DilithiumLevel2},
		{crypto.KyberLevel768, crypto.DilithiumLevel3},
		{crypto.KyberLevel1024, crypto.DilithiumLevel5},
	}
	...
	for _, level := range levels {
		t.Run("SecurityLevel", func(t *testing.T) {
			...
			cm, err := crypto.NewCryptoManager(level.kyber, level.dilithium, logger)
			if err != nil {
				t.Fatal(err)
			}
...
func TestLargePayloadStress(t *testing.T) {
	...
	signKP, err := cm.GenerateSignatureKeyPair()
	if err != nil {
	...
	// Stress signing and verification
	t.Run("SignVerify10MB", func(t *testing.T) {
		sig, err := cm.Sign(signKP, payload)
		if err != nil {
			t.Fatalf("Sign error: %v", err)
		}

		ok, err := cm.Verify(signKP, payload, sig)
		if err != nil || !ok {
			t.Fatalf("Verify error: %v", err)
		}
	})

	// Stress encryption, decryption and verification
	t.Run("EncryptDecryptVerify10MB", func(t *testing.T) {
		ct, enc, sig, err := cm.CombinedEncryptAndSign(kemKP.PublicKey, signKP, payload)
		if err != nil {
			t.Fatalf("CombinedEncryptAndSign error: %v", err)
		}

		recovered, ok, err := cm.CombinedDecryptAndVerify(kemKP.PrivateKey, signKP, ct, enc, sig)
		if err != nil || !ok {
			t.Fatalf("CombinedDecryptAndVerify error: %v", err)
		}
